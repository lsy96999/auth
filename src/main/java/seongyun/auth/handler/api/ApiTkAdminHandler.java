...
public class ApiTkAdminHandler {
	...
	public Mono<ServerResponse> getTkAdminByMail (ServerRequest req) {
		String adminMail = req.pathVariable("adminMail");
		return tkAdminService.getTkAdminByMail(adminMail).flatMap(adm->{
	...
	public Mono<ServerResponse> insertTkAdmin(ServerRequest req){
		...
		return rdto.flatMap(dto -> {
			return tkAdminService.insertTkAdmin(dto).flatMap( adm -> {
				return ok().body(fromValue(RVO.builder()
												.code("0000")
												.message("등록됐습니다.")
												.data(adm)
